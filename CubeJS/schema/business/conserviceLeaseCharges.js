cube(`bus_provider_leaseCharges`, {
    sql: `
        ...
        AND (${FILTER_PARAMS.bus_provider_leaseCharges.businessUnitId.filter((value) => `u.business_unit_id = ${value}`)})
        AND (${FILTER_PARAMS.bus_provider_leaseCharges.startDate.filter((value) => `lease_effective_date >= ${value}`)})
        AND (${FILTER_PARAMS.bus_provider_leaseCharges.endDate.filter((value) => `lease_end_date <= ${value}`)})
        ...
        ,L.LEASE_END_DATE
        ,L.RESIDENT_STATUS
        ,ch.POST_DATE FROM_DATE
        ,LAST_DAY(ch.POST_DATE) THROUGH_DATE
        ,ch.NOTES
        ,ch.POST_DATE POST_MONTH
